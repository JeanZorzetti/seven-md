              ...
              {accountOpen && (
                <div className="absolute right-0 top-full mt-1 min-w-[220px] rounded-lg border border-gray-200 bg-white p-2 shadow-lg z-50">
                  ...
                  {(session.role === 'ADMIN' || session.role === 'SUPER_ADMIN') && (
                    <div className="py-1 border-t border-gray-100">
                      <Link href="/admin" onClick={() => setAccountOpen(false)} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm font-semibold text-[#af101a] hover:bg-red-50 transition-colors">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex flex-col gap-1">
            ...
            {session && (
              <>
                ...
                {(session.role === 'ADMIN' || session.role === 'SUPER_ADMIN') && (
                  <div className="border-t border-gray-100 pt-2">
                    <Link href="/admin" onClick={() => setMobileOpen(false)} className="block rounded-md px-3 py-2 text-sm font-semibold text-[#af101a] hover:bg-red-50">
                      Painel Admin
                    </Link>
                  </div>
                )}
